...
CACHE_DB_PATH = "./Cache/xatom_cache.sqlite"
...
def open_xatom_cache_db(db_path=CACHE_DB_PATH):
    """
    Open SQLite cache database.

    Important for parallel runs:
    - WAL mode allows multiple readers while another process writes.
    ...
    # Generic lock table.
    # This prevents two workers from running the same expensive XATOM job
    # for the same config at the same time.
    conn.execute("""
    ...
    # Case 1:
    # Bound-bound transition search for the original config.
    # One config can have many transition rows, so use a separate done table.
    conn.execute("""
    ...
    # Case 2:
    # Lifetime cache for each excited-state config.
    conn.execute("""
    ...
    # Case 3:
    # Non-resonant PACS-derived f'' cache for each config.
    conn.execute("""
...
def get_bound_transitions_cached(conn, config, output_dir, photonE):
    """
    Case 1 cache:
    ...
    The transition search is now photon-energy independent because PE_range
    is fixed to 490-770 eV.
    """

    h = config_hash(config)

    cached_df = _load_bound_transitions_from_db(conn, config)
    ...
    try:
        ...
        return _load_bound_transitions_from_db(conn, config)
...
def calculate_formfac(config, photonE, temp_storage_folder, cache_db_path=CACHE_DB_PATH):
    """
    Calculate f0, f', and f'' for one electronic configuration and one photon energy.

    New SQL-cache logic
    -------------------
    There are three XATOM run types, and all are now treated as
    photon-energy independent with respect to the XATOM input.

    Case 1:
        Bound-bound transition search.
    ...
    Case 2:
        Lifetime of excited/intermediate configuration.
    ...
    Parallel safety
    ---------------
    Each process opens its own SQLite connection.
    SQLite WAL mode allows parallel readers.
    The cache_locks table prevents different processes from running and
    writing the same config at the same time.
    """

    # Open one SQLite connection inside this function.
    # This is important for multiprocessing: do not share one connection
    # between multiple processes.
    conn = open_xatom_cache_db(cache_db_path)
    ...
    try:
        ...
        # Each photon energy still gets its own temporary XATOM output folder.
        # This prevents raw XATOM output files from different workers from
        ...
        try:
            ...
            for _, row in bound_bound_transitions.iterrows():
                ...
                # XATOM dipole matrix element is assumed to be in atomic units.
                # Convert to meters for the SI formula.
                dipole_matrix_element = float(row["dipole_matrix_element"]) * a0

                # Lifetime of the intermediate excited config has already
                # been obtained from the decay-lifetime cache.
                ...
                # Negative imaginary convention:
                # absorption/resonance gives f'' < 0.
                fpp_i2j = (
                    -prefactor
                    * (Gamma_j / 2)
                    / denominator
            ...
            # Use interpolation instead of exact PhotonEs == photonE.
            # This avoids one-element-array problems and is safer for
            # non-integer or nonuniform energy grids.
